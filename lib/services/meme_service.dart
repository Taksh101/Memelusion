...
class MemeService {
  ...
  print('🔄 shareMeme called [$callId]: memeId=$memeId, sender=$senderUsername, receiver=$receiverUsername');

  // Validate usernames
  if (senderUsername.isEmpty || receiverUsername.isEmpty) {
    print('❌ [$callId] Empty username: sender=$senderUsername, receiver=$receiverUsername');
    return;
  }
  if (senderUsername == receiverUsername) {
    print('❌ [$callId] Cannot share meme to self: sender=$senderUsername, receiver=$receiverUsername');
    return;
  }

  String updatedReceiverUsername = receiverUsername;
  if (receiverUsername.contains(RegExp(r'^[a-zA-Z0-9]{20,}$'))) {
    print('❌ [$callId] receiverUsername appears to be a UID: $receiverUsername');
    try {
      final userDoc = await _firestore.collection('users').doc(receiverUsername).get();
      if (userDoc.exists && userDoc['username'] != null) {
        print('🔄 [$callId] Converting UID $receiverUsername to username ${userDoc['username']}');
      ...
      print('❌ [$callId] Error fetching username for UID $receiverUsername: $e');
      return;
    }
  }

  if (senderUsername == updatedReceiverUsername) {
    print('❌ [$callId] Cannot share meme to self after UID conversion: sender=$senderUsername, receiver=$updatedReceiverUsername');
    return;
  }
  ...
  try {
    ...
    final receiverQuery = await _firestore
        ...
        .limit(1)
        .get();
    final receiverUid = receiverQuery.docs.isNotEmpty ? receiverQuery.docs.first.id : null;
    if (receiverUid == null) {
      print('❌ [$callId] Receiver UID not found for username: $updatedReceiverUsername');
      return;
    }
    if (receiverUid == senderUid) {
      print('❌ [$callId] Cannot share meme to self: senderUid=$senderUid, receiverUid=$receiverUid');
      return;
    }
    ...
    print('📤 [$callId] Sending meme to chats/$chatId/messages: $messageData');
    ...
    final messageRef = await _firestore
        ...
        .collection('messages')
        .add(messageData);
    print('✅ [$callId] Shared meme to room=$chatId, messageId=${messageRef.id}, url=$imageUrl');
    ...
    print('✅ [$callId] Sent notification to $updatedReceiverUsername (uid=$receiverUid)');
  } catch (e) {
    print('❌ [$callId] Error in shareMeme: $e');
    // if (e.toString().contains('permission-denied')) {
    //   print('⚠️ [$callId] Check Firestore rules for chats/$chatId/messages and users/$receiverUid/notifications');
    // }
  }
}
